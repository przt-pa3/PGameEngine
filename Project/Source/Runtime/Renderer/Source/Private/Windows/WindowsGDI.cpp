...
void WindowsGDI::DrawStatisticTexts()
{
	...
	{
		static const int leftPosition = 500;
...
void WindowsGDI::DrawLogs()
{
	if (_PermanentLogs.size())
	{
		HFONT hFont, hOldFont;
		hFont = (HFONT)GetStockObject(ANSI_VAR_FONT);
		if (hOldFont = (HFONT)SelectObject(_MemoryDC, hFont))
		{
			static const int leftPosition = 50;
			static const int topPosition = 10;
			static const int rowHeight = 20;
			int currentPosition = topPosition;
			for (std::vector<std::string>::const_iterator it = _PermanentLogs.begin(); it != _PermanentLogs.end(); ++it)
			{
				TextOut(_MemoryDC, leftPosition, currentPosition, (*it).c_str(), (int)((*it).length()));
				currentPosition += rowHeight;
			}
	...
	if (_TempLogs.size())
	{
		...
		{
			static const int leftPosition = 50;
			static const int topPosition = 400;
			static const int rowHeight = 20;
			int currentPosition = topPosition;
			for (std::vector<std::pair<std::size_t, std::string>>::const_iterator it = _TempLogs.begin(); it != _TempLogs.end(); ++it)
			{
				TextOut(_MemoryDC, leftPosition, currentPosition, (*it).second.c_str(), (int)((*it).second.length()));
				currentPosition += rowHeight;
			}
